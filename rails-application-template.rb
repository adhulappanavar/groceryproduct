...
puts
say_status  "Model", "Adding the Groceryproduct resource...", :yellow
puts        '-'*80, ''; sleep 1

generate :scaffold, "Groceryproduct title:string content:text published_on:date"
route "root :to => 'groceryproducts#index'"
rake  "db:migrate"

git :add => '.'
git :commit => "-m 'Added the Groceryproduct resource'"
...
puts
say_status  "Model", "Adding search support into the Groceryproduct model...", :yellow
puts        '-'*80, ''; sleep 1

run "rm -f app/models/groceryproduct.rb"
file 'app/models/groceryproduct.rb', <<-CODE
...
git :commit => "-a -m 'Added Tire support into the Groceryproduct class and an initializer'"

puts
say_status  "Controller", "Adding controller action, route, and HTML for search...", :yellow
puts        '-'*80, ''; sleep 1

gsub_file 'app/controllers/groceryproducts_controller.rb', %r{# GET /groceryproducts/1$}, <<-CODE
...
gsub_file 'app/views/groceryproducts/index.html.erb', %r{<h1>Listing groceryproducts</h1>}, <<-CODE
<h1>Listing groceryproducts</h1>

<%= form_tag search_groceryproducts_path, :method => 'get' do %>
  <%= label_tag :query %>
...
gsub_file 'app/views/groceryproducts/index.html.erb', %r{<%= link_to 'New Groceryproduct', new_groceryproduct_path %>}, <<-CODE
<%= link_to 'New Groceryproduct', new_groceryproduct_path %>
<%= link_to 'Back', groceryproducts_path if params[:q] %>
CODE

gsub_file 'config/routes.rb', %r{resources :groceryproducts}, <<-CODE
resources :groceryproducts do
...
rake "environment tire:import:model CLASS='Groceryproduct' FORCE=true"
